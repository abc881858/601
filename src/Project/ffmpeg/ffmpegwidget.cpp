...
int FFmpegWidget::start_preview(const std::string &media_url)
{
    return FFmpegPlayer::start_preview(media_url);
}

int FFmpegWidget::stop_preview()
{
    return FFmpegPlayer::stop_preview();
}

void FFmpegWidget::initializeGL()
{
...
void FFmpegWidget::setup_viewport(int w, int h)
{
    if(!w||!h)
        return;
...
void FFmpegWidget::resizeGL(int w, int h)
{
...
void FFmpegWidget::on_preview_start(const std::string& media_url, const int width, const int height)
{
...
void FFmpegWidget::on_preview_stop(const std::string& media_url)
{
    qDebug() << __FUNCTION__ ;
    aspectRatio = 0;
...
void FFmpegWidget::on_recorder_start(const std::string& file)
{
    qDebug() << __FUNCTION__ ;
    emit sig_on_record_start(QString(file.c_str()));
}

void FFmpegWidget::on_recorder_stop(const std::string& file)
{
    qDebug() << __FUNCTION__ ;
    emit sig_on_record_stop(QString(file.c_str()));
}

void FFmpegWidget::on_new_frame_avaliable(std::shared_ptr<FrameCache> m_frame_cache)
{
    QMetaObject::invokeMethod(this, [&](){
        this->update();
    }, Qt::QueuedConnection);
